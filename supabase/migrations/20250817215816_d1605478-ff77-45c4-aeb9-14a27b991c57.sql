...
-- Update existing membros table to match requirements
ALTER TABLE public.membros 
  DROP COLUMN IF EXISTS terreiro_id,
  ADD COLUMN IF NOT EXISTS org_id UUID NOT NULL REFERENCES public.terreiros(id) ON DELETE CASCADE,
...
ALTER TABLE public.planos 
  DROP COLUMN IF EXISTS terreiro_id,
  ADD COLUMN IF NOT EXISTS org_id UUID NOT NULL REFERENCES public.terreiros(id) ON DELETE CASCADE,
  ALTER COLUMN valor TYPE INTEGER USING (valor * 100)::INTEGER,
  ADD COLUMN IF NOT EXISTS valor_centavos INTEGER NOT NULL CHECK (valor_centavos > 0),
  ADD CONSTRAINT check_dia_vencimento CHECK (dia_vencimento BETWEEN 1 AND 28);

-- Update existing assinaturas table  
ALTER TABLE public.assinaturas
  DROP COLUMN IF EXISTS terreiro_id,
  ADD COLUMN IF NOT EXISTS org_id UUID NOT NULL REFERENCES public.terreiros(id) ON DELETE CASCADE,
  ADD COLUMN IF NOT EXISTS inicio DATE NOT NULL DEFAULT CURRENT_DATE,
  ADD COLUMN IF NOT EXISTS fim DATE,
  ADD COLUMN IF NOT EXISTS status TEXT CHECK (status IN ('ativa','pausada','cancelada')) DEFAULT 'ativa';

-- Update existing faturas table
ALTER TABLE public.faturas
  DROP COLUMN IF EXISTS terreiro_id,
  ADD COLUMN IF NOT EXISTS org_id UUID NOT NULL REFERENCES public.terreiros(id) ON DELETE CASCADE,
  ADD COLUMN IF NOT EXISTS refer CHAR(6) NOT NULL,
  ADD COLUMN IF NOT EXISTS dt_vencimento DATE NOT NULL,
  ALTER COLUMN valor TYPE INTEGER USING (valor * 100)::INTEGER,
  ADD COLUMN IF NOT EXISTS valor_centavos INTEGER NOT NULL CHECK (valor_centavos > 0),
