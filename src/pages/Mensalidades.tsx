...
export default function Mensalidades() {
  ...
  const myMembroId = profile?.membro_id ?? null;
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // se não tiver sessão ainda, espera próximo render
        ...
        if (isViewer) {
          ...
          if (!linked) {
            ...
            return; // cai no finally → loading=false
          }
  ...
  const buscar = async () => {
    try {
      ...
      if (isViewer) {
        // em modo viewer ignoramos o campo de busca e usamos o próprio membro
        if (!myMembroId) {
  ...
  useEffect(() => {
    if (selectAll) {
      // só seleciona as não pagas para pagamento em lote
              ...
              <div className="space-y-2">
                ...
                {isViewer ? (
                  ...
                  <div className="flex items-center gap-2">
                    <Select
                      value={payPreset}
                      onValueChange={(v: any) => setPayPreset(v)}
                    >
                      <SelectTrigger className="w-56">
