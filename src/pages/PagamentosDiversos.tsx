...
interface PagDiverso {
  id: string;
  tipo: string; // doacao | compra | outro
...
export default function PagamentosDiversos() {
  ...
  // Se viewer, force PIX e métodos, e trave matrícula = própria
  useEffect(() => {
    if (isViewer) {
      setForm(f => ({
        ...f,
      ...
      setMetodos([{ id: 'preset-pix', nome: 'pix' }]); // limita visualmente
      // pré-resolver membro do viewer (se tiver matrícula)
      if (myMatricula) {
        void resolveMembro(myMatricula, /*silent*/ true);
      }
    }
  ...
  const ensureValidOrgId = async (): Promise<string> => {
    ...
    const exists = async (id: string | null) => {
      if (!id) return false;
      const { data } = await supabase.from('terreiros').select('id').eq('id', id).maybeSingle();
      return !!data;
    };
  ...
  const submitAddTipo = async () => {
    ...
    try {
      ...
      if (!orgId) await ensureValidOrgId();
      ...
      await supabase.from('pagamentos_diversos_tipos').insert({ terreiro_id: orgId, nome });
      await loadTipos(orgId!);
  ...
  const submitAddMetodo = async () => {
    ...
    try {
      ...
      await supabase.from('pagamentos_diversos_metodos').insert({ terreiro_id: orgId, nome });
      await loadMetodos(orgId!);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Viewer só pode PIX e só a própria matrícula
      const metodoFinal = isViewer ? 'pix' : form.metodo;

      // Resolve/valida matrícula final
      ...
      const payload = {
        ...
        observacoes: normalize(form.observacoes || ''), // << salva no BD
      };
