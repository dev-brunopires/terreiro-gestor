...
/** util: cria signed url (ou string vazia) */
async function signedUrlOrEmpty(bucket: string, path: string, seconds = 60 * 60) {
...
========================= */
/* =========================
   Seção: Meu Perfil (com avatar via signed URL)
========================= */
function SectionPerfil() {
  ...
  const SIGNED_REFRESH_MS = 55 * 60 * 1000; // renova antes de expirar

  const isHttp = (s?: string | null) => !!s && /^https?:\/\//i.test(s);
  const bust = (u: string) => {
    try {
      const x = new URL(u);
      x.searchParams.set("t", Date.now().toString());
      return x.toString();
    } catch {
      return u;
    }
  ...
  /** Cria/atualiza a URL exibida (assina path ou usa pública com cache-bust) */
  const setDisplayAvatarFromRaw = async (raw?: string | null) => {
    ...
    if (isHttp(raw)) {
      // legado: já é URL pública
      setAvatarUrl(bust(raw));
      return;
    }

    // esperado: path no bucket avatars
    const signed = await signedUrlOrEmpty("avatars", raw, 60 * 60);
    setAvatarUrl(signed || undefined);

    // agenda renovação (caso usuário deixe a tela aberta)
    refreshTimer.current = window.setInterval(async () => {
      const s = await signedUrlOrEmpty("avatars", raw, 60 * 60);
      setAvatarUrl(s || undefined);
    }, SIGNED_REFRESH_MS);
  };

  useEffect(() => setNome(profile?.nome ?? ""), [profile?.nome]);

  // carrega avatar existente (suporta legado: URL pública ou path)
  useEffect(() => {
    const meta = (user?.user_metadata as any) || {};
    const raw: string | undefined =
      (profile as any)?.avatar_url ||
      meta?.avatar_path ||      // preferir path se existir
      meta?.avatar_url ||       // pode ser path legado salvo com esse nome
      undefined;

    (async () => {
      await setDisplayAvatarFromRaw(raw);
    })();
  ...
  const onFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // upsert no bucket privado/público
      const { error: upErr } = await supabase.storage
        .from("avatars")
        .upload(path, file, { upsert: true });
      if (upErr) throw upErr;

      // Salva apenas o PATH no profile (não a URL assinada)
      ...
      // Guarda path também no user_metadata (para outras áreas da UI)
      const { error: metaErr } = await supabase.auth.updateUser({
        data: { avatar_path: path, avatar_url: path },
      });
      if (metaErr) throw metaErr;

      // Gera signed URL para exibir agora
      await setDisplayAvatarFromRaw(path);
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Suporte a PNG/JPG até 2MB. (bucket: <code>avatars</code>)
              </p>
            </div>
          </div>
...
function SectionOrganizacao({ canEdit }: { canEdit: boolean }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setLoading(true);
      // buscamos logo_bucket/logo_path (privado) e logo_url (compat público)
      const { data, error } = await supabase
        .from("terreiros")
        .select("*, logo_bucket, logo_path, logo_url")
      ...
      if (data) {
        ...
        // prioridade: signed URL se tivermos bucket/path; senão usa logo_url (público)
        if (d.logo_bucket && d.logo_path) {
          const url = await signedUrlOrEmpty(d.logo_bucket, d.logo_path);
          setLogoUrl(url || "");
        } else {
  ...
  const onFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // ⚠️ não salve signed url no BD (expira). Grave bucket/path e zere logo_url (compat).
      const { error: updErr } = await supabase
        .from("terreiros")
        .update({ logo_bucket: bucket, logo_path: path, logo_url: null })
        .eq("id", profile.org_id);
      if (updErr) throw updErr;

      // gera URL assinada só para exibir agora
      const url = await signedUrlOrEmpty(bucket, path);
      setLogoUrl(url || "");
  ...
  const removeLogo = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        .from("terreiros")
        .update({ logo_url: null, logo_bucket: null, logo_path: null })
        .eq("id", profile.org_id);
      if (error) throw error;
      setLogoUrl("");
  ...
  const salvar = async () => {
    ...
    try {
      const payload = {
        nome,
        // não salvar logoUrl (signed) no banco
        email: email || null,
